...
router.post("/handleTask", async (ctx) => {
  ...
  logger.debug(
    { trace_id: ctx.trace.gcpTrace, span_id: ctx.trace.spanId, task },
    "Sending task to Cloud Tasks",
  );
...
router.post("/receivedTask", async (ctx) => {
  ...
  logger.debug(
    { trace_id: ctx.trace.gcpTrace, span_id: ctx.trace.spanId, taskData },
    "Processing task",
  );
  ...
  try {
    ...
    logger.error(
      { trace_id: ctx.trace.gcpTrace, span_id: ctx.trace.spanId, error },
      "Error processing task, will retry",
    );
    ctx.status = 500;
    ctx.body = {
      status: "ERROR",
...
router.post("/test", async (ctx) => {
  logger.info(
    { trace_id: ctx.trace.gcpTrace, span_id: ctx.trace.spanId, action: "test" },
    "Test endpoint called",
  );
  logger.debug(
    { trace_id: ctx.trace.gcpTrace, span_id: ctx.trace.spanId },
    "Debug info with trace context",
  );
  logger.warn(
    { trace_id: ctx.trace.gcpTrace, span_id: ctx.trace.spanId },
    "This is a warning log for testing",
  );
...
router.get("/single", async (ctx) => {
  ...
  logger.info(
    { action: "multisimple_test" },
    "Handling /single request testing",
  );
...
router.get("/multi", async (ctx) => {
  ...
  logger.info({ action: "multi" }, "Handling /multi request action");
